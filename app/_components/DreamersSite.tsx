...
const copy = {
  en: {
    ...
    openMap: "Open in Google Maps", contact: "Message us", rules: "Good to know", rulesTitle: "House notes.", checkin: "Check-in", checkout: "Check-out", children: "Children", childrenValue: "All ages welcome", payment: "Payment", paymentValue: "Cash at property", pets: "Pets", petsValue: "On request", parties: "Parties", partiesValue: "No events or bachelor parties", deposit: "Damage deposit", depositValue: "AMD 20,000 on arrival", policyNote: "Cancellation and prepayment conditions vary by room and rate. Confirm the latest terms when booking.",
  ...
  ru: {
    ...
    openMap: "Открыть в Google Maps", contact: "Написать нам", rules: "Полезно знать", rulesTitle: "Правила дома.", checkin: "Заезд", checkout: "Выезд", children: "Дети", childrenValue: "Любого возраста", payment: "Оплата", paymentValue: "Наличными на месте", pets: "Питомцы", petsValue: "По запросу", parties: "Вечеринки", partiesValue: "Мероприятия запрещены", deposit: "Залог", depositValue: "20 000 AMD при заезде", policyNote: "Условия отмены и предоплаты зависят от номера и тарифа. Уточняйте актуальные условия при бронировании.",
...
function Footer({ lang, c }: { lang: Language; c: typeof copy.en }) {
  return <footer><div className="footer-top"><div><Link className="footer-brand" href="/">Dreamers<span>✦</span></Link><p>{c.footerLine}</p></div><div><small>{c.direct}</small><a href="tel:+37493847050">+374 93 847050</a><a href={whatsappUrl()} target="_blank" rel="noreferrer">WhatsApp <Arrow /></a></div><div><small>{c.navRooms}</small>{rooms.map((room) => <Link href={`/rooms/${room.slug}`} key={room.slug}>{room.name[lang]}</Link>)}</div></div><div className="footer-bottom"><span>{c.rights}</span><span>68 Sarmen Street · 0014</span></div></footer>;
}
